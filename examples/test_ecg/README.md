...
## 1.4 Python Package Installation

```
pip install -r examples/test_ecg01/requirements.txt
```

`wfdb` is required by `data_prepare_win.py` to read MIT-BIH records and is listed in `requirements.txt`.
...
## 2.6 Example Directory Structure

The ECG model definition is placed under a `model/` subdirectory, consistent with the other examples:
...
All commands below are intended to be run from the latti-ai repository root. The scripts add the repository root to `sys.path` automatically, so `PYTHONPATH=. python examples/test_ecg01/train.py` is not required.
...
```
python examples/test_ecg01/data_prepare_win.py \
...
Note: Data processing can run on Windows or Linux. Pass the local MIT-BIH dataset path through `--data-dir`; no source-code path edit is required.
